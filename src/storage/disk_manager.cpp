...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    ...
    if (fd < 0 || page_no < 0 || offset == nullptr || num_bytes <= 0) {
        throw UnixError();
    }
    // 计算目标页面在文件中的偏移量 = 页号 * 页大小
...
void DiskManager::create_file(const std::string &path) {
    ...
    int fd = open(path.c_str(), O_CREAT, S_IRUSR | S_IWUSR);  // 创建一个名为path的文件，权限为可读和可写
    if (fd < 0) {  // 如果创建失败
        throw UnixError();
    }
    close(fd);  // 关闭文件描述符

    if (!is_file(path)) {  // 如果文件不存在
...
void DiskManager::destroy_file(const std::string &path) {
    ...
    if (path2fd_.count(path)) {
        throw FileNotClosedError(path);  // 如果文件仍然打开，抛出异常
    }
    ...
    if (unlink(path.c_str()) < 0) {  // 删除名为path的文件
        throw UnixError();
    }

    if (is_file(path)) {  // 如果文件仍然存在
...
int DiskManager::open_file(const std::string &path) {
    ...
    auto path_fd_pair = path2fd_.find(path);
    if (path_fd_pair != path2fd_.end()) {
        return path_fd_pair->second;
    }
    ...
    int fd = open(path.c_str(), O_RDWR);  // 打开一个名为path的文件，权限为可读和可写
    if (fd < 0) {  // 如果打开失败
        throw UnixError();
    }
...
void DiskManager::close_file(int fd) {
    ...
    auto fd_path_pair = fd2path_.find(fd);
    if (fd_path_pair == fd2path_.end()) {
    ...
    // 从打开列表中移除
    std::string path = fd_path_pair->second;
    fd2path_.erase(fd);
    path2fd_.erase(path);
}
